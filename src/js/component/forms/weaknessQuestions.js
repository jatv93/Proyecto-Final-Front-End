	...
	return (
		<Fragment>
			<Modal show={modalShow} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
				...
				<form onSubmit={e => actions.submitTeacherWeaknessQuestion(e)}>
					<Modal.Body>
						<textarea
							...
							rows="3"
							required
							onChange={e => actions.addTeacherWeaknessQuestion(e.target.value)}
						/>
					</Modal.Body>
					<Modal.Footer>
							...
							<ul key={index}>
								<li>
									...
									<FontAwesomeIcon
										icon={faTrash}
										className="mr-2"
										onClick={() => actions.deleteTeacherWeaknessQuestion(index)}
									/>
